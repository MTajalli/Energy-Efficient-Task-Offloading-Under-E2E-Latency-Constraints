function [Acceptance_Ratio, Tau_tx, Tau_exe, Tau_prop, Radio_Cost, Computation_Cost] = JTO_function(max_delay, params)
%% JTO.m - Joint Task Offloading and Resource Allocation
% This script implements a joint optimization framework for task offloading,
% power allocation, and resource allocation in wireless networks with
% mobile edge computing (MEC) capabilities.
%
% The algorithm:
% 1. Feasibility analysis to find a feasible subset of tasks
% 2. Power allocation via Convex-Concave Programming (CCP)
% 3. Computational resource allocation via CVX
% 4. Slack variable computation via CVX
% 5. Heuristic task placement refinement
% 6. Final optimization for the feasible task set
...
% disp('FEASIBILITY ANALYSIS');


Tau = max_delay * ones(K, 1);  % Delay deadlines for each task

%% ========== CHANNEL AND NOISE ==========

% H = randn(N_ant, L_R, K) + 1i * randn(N_ant, L_R, K);
sigma = 1;  % Noise power

%% ========== INITIAL TASK PLACEMENT ==========
K_set = 1:K;                    % Set of all tasks
v = zeros(N, length(K_set));    % Task placement indicator
e = zeros(N, max(path), length(K_set));  % Path assignment

% Initially place all tasks at the source node (node 1)
for k = K_set
    e(1, 1, k) = 1;
end
...
p_var = 1e-4 * ones(length(K_set), 1);  % Initial transmit power
...
% Initialize slack variables (large initial value)
s = Tau_tx + Tau_prop + 1000 * max(Tau) * ones(length(K_set), 1);

%% ========== FEASIBILITY ANALYSIS ==========
I_max = 100;

while true  % Main feasibility loop
    % disp(['Number of Feasible Tasks: ', num2str(length(K_set))]);

    count = 0;
    elastic_stack = zeros(length(K_set), I_max);

    while true  % ASM for given task set
        convergence_check = sum(s);
        count = count + 1;
        % disp(['ASM Iteration: ', num2str(count)]);
        ...
        % disp('Computational Resource Allocation');
        cvx_begin
        ...
        while true
            ...
            for k = K_set
                ...
                log(sum(temp * p) + sigma^2) / log(2) - g(k) - nabla_g(k, :) * (p - p_var) >= ...
                    D(k) / (Tau(k) + s(k) - Tau_prop(k) - Tau_exe(k));
                p(k) >= 0;
                p(k) <= 0.5;
            end

            % Link capacity constraints (C3)
            for m = 1:N
                for mm = 1:N
                    temp = reshape(Ie(m, mm, :), [1, length(K_set)]);
                    temp * (h + nabla_h * (p - p_var) - ...
                        log(X * p - diag(Y) * p + sigma^2 * ones(length(K_set), 1)) / log(2)) <= C_link(m, mm);
                end
            end

            % Front-haul capacity constraints (C4)
            for l = 1:L_R
        ...
        % disp('Computing Slack Variables');
        cvx_begin
        variable s_new(length(K_set))
        minimize(sum(s_new))
        subject to
        for k = K_set
            s_new(k) >= Tau_prop(k) + Tau_exe(k) + Tau_tx(k) - Tau(k);
            s_new(k) >= 0;
        end
        cvx_end
        s = s_new;
        ...
        for k = Sorted_Tasks'
            ...
            for m = 1:N
                for mm = 1:N
                    ...
                    for i = K_set
                        if i ~= k
                            temp2 = reshape(r_aug(:, :, i), [N, max(path)])';  % Now [max(path), N]
                            temp(i) = sum(sum(temp1 .* e(:, :, i)' .* temp2));
                        end
                    end
            ...
            s_tilde = Tau_tx(k) + Tau_prop(k) + L(k) / C_tilde_node(n_star) - Tau(k);
            if s_tilde < s(k) / 2
                c(k) = L(k) / (Tau(k) + s(k) / 2 - Tau_tx(k) - Tau_prop(k));
                s(k) = s(k) / 2;
            else
                s(k) = s_tilde;
...
Acceptance_Ratio = length(K_set) / K;
...
while true  % Final optimization loop
    count = count + 1;
    % disp(['Optimization Iteration: ', num2str(count)]);
    ...
    while true
        count_p = count_p + 1;
        % disp(['CCP Iteration: ', num2str(count_p)]);
        % fprintf('Delay Budget for Power Allocation = %s\n', num2str((Tau - Tau_prop - Tau_exe)', '%.6f '))
        cvx_begin
        ...
        for k = K_set
            temp = reshape(h_tilde(k, :, :), [L_R, length(K_set)]);
            log(sum(temp * p) + sigma^2) / log(2) - g(k) - nabla_g(k, :) * (p - p_var) >= ...
                D(k) * inv_pos(Tau(k) - Tau_prop(k) - Tau_exe(k) + alpha_p);
            p(k) >= 0;
            p(k) <= 0.5;
        end
        alpha_p >= 0;
        ...
        cvx_end
        % fprintf('Power Slack Variable %s\n', num2str(alpha_p, '%.6f '))
    ...
    %% ===== COMPUTATIONAL RESOURCE ALLOCATION =====
    % disp('Computational Resource Allocation');
    % fprintf('Delay Budget for Computing Allocation = %s\n', num2str((Tau - Tau_prop - Tau_tx)', '%.6f '))
    cvx_begin
    ...
    cvx_end
    % fprintf('Computing Slack Variable %s\n', num2str(alpha_c, '%.6f '))
    ...
    Computation_Cost = 1e-1 * sum(c_n.^3);
    Radio_Cost = cost_power;
    ovreall_cost(count) = cost_power + 1e-1 * cost_computation;

    if count > 1
        if abs(ovreall_cost(count - 1) - ovreall_cost(count)) < 1e-3 || count >= I_max
            break;
        end
    end
end

% disp(['Total Cost: ', num2str(ovreall_cost(count))]);
end